  ...
  UPDATE products p
     SET "searchVector" = (
       SELECT
         ...
         setweight(to_tsvector('simple',      coalesce(p2.sku, '')),         'B') ||
         setweight(to_tsvector('fr_unaccent', coalesce(b.name, '')),         'C') ||
         setweight(to_tsvector('fr_unaccent', coalesce(ct.name, '')),        'C') ||
         setweight(to_tsvector('fr_unaccent', coalesce(pct.name, '')),       'C') ||
       ...
       LEFT JOIN brands b
         ON b.id = p2."brandId"
       -- Catégorie du produit, et sa rubrique parente : chercher « électronique »
       -- doit remonter un produit rangé dans « Électronique > Audio ».
       LEFT JOIN categories c
         ON c.id = p2."categoryId"
       LEFT JOIN category_translations ct
         ON ct."categoryId" = c.id AND ct.locale = 'fr'
       LEFT JOIN category_translations pct
         ON pct."categoryId" = c."parentId" AND pct.locale = 'fr'
       WHERE p2.id = p.id
     )
...
/**
 * Recalcule tous les produits rattachés à une catégorie, directement ou via une
 * sous-catégorie. Appelé quand une catégorie est renommée : sans cela, le vecteur
 * de recherche garderait l'ancien nom indéfiniment.
 *
 * Opération rare (les catégories bougent peu) et volontairement simple.
 */
CREATE OR REPLACE FUNCTION beralshopp_refresh_category_search(p_category_id text)
RETURNS void
LANGUAGE plpgsql
AS $$
DECLARE
...
-- Déclencheur des renommages de catégorie.
CREATE OR REPLACE FUNCTION beralshopp_trg_refresh_category_search()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    target_id := OLD."categoryId";
  ELSE
    target_id := NEW."categoryId";
  END IF;

  IF target_id IS NOT NULL THEN
    PERFORM beralshopp_refresh_category_search(target_id);
  END IF;
...
AFTER INSERT OR UPDATE OF sku, "brandId", "categoryId" ON products
...
DROP TRIGGER IF EXISTS trg_category_translations_search ON category_translations;
CREATE TRIGGER trg_category_translations_search
AFTER INSERT OR UPDATE OR DELETE ON category_translations
FOR EACH ROW EXECUTE FUNCTION beralshopp_trg_refresh_category_search();
