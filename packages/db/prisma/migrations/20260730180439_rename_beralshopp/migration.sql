-- ═══════════════════════════════════════════════════════════════════════════════
--  Renommage de la marque : Beralshop → Beralshopp (orthographe officielle du logo).
--
--  Aucune perte de données : on renomme la séquence (donc le compteur de numéros de
--  commande est PRÉSERVÉ), on supprime les anciennes fonctions et triggers, puis on
--  recrée l'ensemble sous les nouveaux noms à partir de
--  prisma/sql/001_search_and_sequences.sql.
-- ═══════════════════════════════════════════════════════════════════════════════

-- 1. La séquence est renommée, jamais recréée : la recréer repartirait à 1 et
--    deux commandes pourraient un jour porter le même numéro.
ALTER SEQUENCE IF EXISTS beralshop_order_number_seq RENAME TO beralshopp_order_number_seq;

-- 2. Anciens triggers et fonctions.
...
DROP FUNCTION IF EXISTS beralshop_trg_refresh_product_search();
DROP FUNCTION IF EXISTS beralshop_refresh_product_search(text);
DROP FUNCTION IF EXISTS beralshop_next_order_number();

-- 3. Le vendeur par défaut suit la marque.
UPDATE vendors SET slug = 'beralshopp', name = 'Beralshopp' WHERE slug = 'beralshop';

-- 4. Recréation complète sous les nouveaux noms (contenu idempotent).
-- ═══════════════════════════════════════════════════════════════════════════════
